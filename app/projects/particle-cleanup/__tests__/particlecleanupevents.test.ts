...
describe('useParticleCleanupEvents', () => {
  ...
  it('should add and remove event listeners correctly', () => {
    const addEventListenerSpy = jest.spyOn(refs.current.container, 'addEventListener');
    const removeEventListenerSpy = jest.spyOn(refs.current.container, 'removeEventListener');
    ...
    expect(windowAddEventListenerSpy).toHaveBeenCalledWith('wheel', handleScroll, { passive: false });
    ...
    expect(windowRemoveEventListenerSpy).toHaveBeenCalledWith('wheel', handleScroll);
  ...
  it('should resize the canvas on window resize', () => {
    ...
    jest.spyOn(refs.current.container, 'getBoundingClientRect').mockReturnValue({
      width: 500,
      height: 300,
      top: 0,
      left: 0,
