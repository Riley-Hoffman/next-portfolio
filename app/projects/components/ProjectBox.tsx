...
export const ProjectBox: React.FC<ProjectBoxProps> = ({
  ...
  return (
    <li className={`relative my-28 border-t-8 border-solid gap-14 gradient-border ${inverted} transition-all duration-1000 ease ${isFirst ? 'mt-12' : 'mt-0'} even:flex-row-reverse md:flex group`} ref={(el) => { if (el) { elementsRef.current.push(el); } }}>
      {children}
