...
export const NavListItem = ({ to = '', label = '', hide = false, resume = '', isActive }: NavListItemProps) =>  {
    ...
    return (
        <li className="block md:inline">
            {resume ? (
                ...
                <Link className={`button ${commonClasses} ${hiddenClass} ${isActive ? 'active' : ''} hover:[&.active]:brightness-100 [&.active]:bg-pink-200 [&.active]:text-zinc`} href={to}>{label}</Link>
            )}
        </li>
    );
};